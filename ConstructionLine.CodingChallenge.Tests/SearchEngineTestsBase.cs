    ...
    public class SearchEngineTestsBase
    {
        ...
        protected static void AssertSizeCounts(List<Shirt> shirts, SearchOptions searchOptions, List<SizeCount> sizeCounts)
        {
            ...
            foreach (var size in Size.All)
            {
                ...
                var expectedSizeCount = shirts.Count(shirt => shirt.Size.Id == size.Id && (!searchOptions.Sizes.Any() || searchOptions.Sizes.Select(siz => siz.Id).Contains(shirt.Size.Id)) && (!searchOptions.Colors.Any() || searchOptions.Colors.Select(color => color.Id).Contains(shirt.Color.Id)));
          
                Assert.That(sizeCount.Count, Is.EqualTo(expectedSizeCount), $"Size count for '{sizeCount.Size.Name}' showing '{sizeCount.Count}' should be '{expectedSizeCount}'");
            }
        }
        ...
        protected static void AssertColorCounts(List<Shirt> shirts, SearchOptions searchOptions, List<ColorCount> colorCounts)
        {
            ...
            foreach (var color in Color.All)
            {
                ...
                var expectedColorCount = shirts.Count(shirt => shirt.Color.Id == color.Id && (!searchOptions.Colors.Any() || searchOptions.Colors.Select(col => col.Id).Contains(shirt.Color.Id))  && (!searchOptions.Sizes.Any() || searchOptions.Sizes.Select(size => size.Id).Contains(shirt.Size.Id)));

                Assert.That(colorCount.Count, Is.EqualTo(expectedColorCount), $"Color count for '{colorCount.Color.Name}' showing '{colorCount.Count}' should be '{expectedColorCount}'");
            }
        }
    }
}